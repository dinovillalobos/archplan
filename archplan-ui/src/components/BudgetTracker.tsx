...
import { X, Wallet, Plus, Receipt, AlertCircle, Edit2, Check, Trash2 } from 'lucide-react';

interface BudgetTrackerProps {
  proyecto: any;
  isOpen: boolean;
  onClose: () => void;
  onUpdate: () => void; // <--- RECIBIMOS EL CABLE
}

export default function BudgetTracker({ proyecto, isOpen, onClose, onUpdate }: BudgetTrackerProps) {
  ...
  const handleAgregarGasto = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`http://localhost:8080/api/gastos/proyecto/${proyecto.id}`, {
        ...
        body: JSON.stringify({ concepto: nuevoConcepto, monto: parseFloat(nuevoMonto), categoria: nuevaCategoria })
      });
      ...
      if (res.ok) {
        ...
        setNuevoMonto('');
        await cargarGastos();
        onUpdate(); // <--- AVISAMOS A APP.TSX QUE ACTUALICE LOS NÚMEROS
      }
          ...
          <div className="col-span-1 flex flex-col gap-5 overflow-y-auto pr-2 pb-4 custom-scrollbar">
              ...
              {isEditingBudget ? (
                <div className="flex items-center gap-2 mt-1">
                  <div className="relative flex-1">
                    <Wallet size={16} className="absolute left-2 top-1.5 text-gray-500" />
                    <input 
                      type="number" autoFocus value={tempBudget} onChange={(e) => setTempBudget(e.target.value)}
            ...
            <div className="grid grid-cols-2 gap-4 shrink-0">
              <div className="bg-arch-card p-4 rounded-xl border border-gray-700">
                <p className="text-xs text-arch-text-gray uppercase tracking-wider mb-1">Gastado</p>
            ...
            <div className="bg-gray-800/30 p-5 rounded-xl border border-gray-700 mt-2 shrink-0">
              <h4 className="text-white font-bold mb-4 flex items-center gap-2">
                <Plus size={18} className="text-arch-blue" /> Registrar Gasto
              </h4>
              <form onSubmit={handleAgregarGasto} className="space-y-4">
                <div>
                  <label className="block text-xs text-arch-text-gray mb-1">Concepto</label>
                  <input type="text" required value={nuevoConcepto} onChange={(e) => setNuevoConcepto(e.target.value)} placeholder="Ej. Acero" className="w-full bg-arch-dark border border-gray-600 rounded px-3 py-2 text-white text-sm focus:border-arch-blue outline-none" />
                </div>
                <div>
                  <label className="block text-xs text-arch-text-gray mb-1">Monto</label>
                  <div className="relative">
                    <Wallet size={14} className="absolute left-3 top-2.5 text-gray-500" />
                ...
                <div>
                  <label className="block text-xs text-arch-text-gray mb-1">Categoría</label>
          ...
          <div className="col-span-1 lg:col-span-2 border border-gray-700 rounded-xl overflow-hidden flex flex-col bg-arch-dark/30">
            <div className="bg-gray-800/50 p-4 border-b border-gray-700 shrink-0">
              <h4 className="text-white font-bold text-sm flex items-center gap-2">
                <Receipt size={16} className="text-arch-text-gray" /> Historial de Transacciones
              </h4>
            </div>
